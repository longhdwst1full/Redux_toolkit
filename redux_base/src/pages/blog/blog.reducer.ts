...
export const deletePost = createAsyncThunk('blog/deletePost', async (id: string, thunkAPI) => {
  const response = await http.delete<Post>(`posts/${id}`, { signal: thunkAPI.signal })
  return response.data
})
  ...
  extraReducers(buider) {
    buider
      ...
      .addCase(deletePost.fulfilled, (state, action) => {
        // meta.arg : là tham số đầu tiên truyền vào hàm vd như async (id: string, thunkAPI) là id
        const postId = action.meta.arg
        const deletePostIndex = state.postList.findIndex((post) => post.id === postId)
        // -1 là không tìm thấy
        if (deletePostIndex !== -1) {
          state.postList.slice(deletePostIndex, 1)
        }
      })
